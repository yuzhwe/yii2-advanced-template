...
class ActionColumn extends Column
{
    ...
    protected function initDefaultButtons()
    {
        if (!isset($this->buttons['view'])) {
            $this->buttons['view'] = function ($url, $model, $key) {
                ...
                return Html::a('<span class="glyphicon">'.(isset($this->hrefText['view']) ? $this->hrefText['view'] : Yii::t('yii', 'View')).'</span>', $url, $options);
            };
        }
        if (!isset($this->buttons['update'])) {
            $this->buttons['update'] = function ($url, $model, $key) {
                ...
                return Html::a('<span class="glyphicon">'.(isset($this->hrefText['update']) ? $this->hrefText['update'] : Yii::t('yii', 'Update')).'</span>', $url, $options);
            };
        }
        if (!isset($this->buttons['delete'])) {
            $this->buttons['delete'] = function ($url, $model, $key) {
                ...
                return Html::a('<span class="glyphicon">'.(isset($this->hrefText['delete']) ? $this->hrefText['delete'] : Yii::t('yii', 'Delete')).'</span>', $url, $options);
            };
        }
        foreach(array_keys($this->buttons) as $r) {
            $route = Yii::$app->controller->uniqueId.'/'. $r;
            if(!\common\modules\rbac\components\Helper::checkRoute($route))
            {
                unset($this->buttons[$r]);
            }
        };
    }
    ...
    public function createUrl($action, $model, $key, $index)
    {
        if (is_callable($this->urlCreator)) {
            ...
            $redirect = urldecode(Yii::$app->request->getAbsoluteUrl());
